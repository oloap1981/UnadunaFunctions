...
public class UnaDunaSendMail implements RequestHandler<RichiestaOtherGenerica, RispostaOtherGenerica> {
	...
	public RispostaOtherGenerica handleRequest(RichiestaOtherGenerica request, Context context) {
		...
		properties.put("mail.smtp.host", hostProperty.getValore());
		...
		try {
			...
			if(ccAddressList != null) {
				for(int i = 0; i < ccAddressList.size(); i++) {
					message.addRecipients(Message.RecipientType.CC, InternetAddress.parse(toAddressList.get(i)));
				}
			}
			...
			transport.connect(hostProperty.getValore(), Constants.MAIL_USER, Constants.MAIL_PASSWORD);
