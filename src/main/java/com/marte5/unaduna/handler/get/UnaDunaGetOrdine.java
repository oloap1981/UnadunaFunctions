...
public class UnaDunaGetOrdine implements RequestHandler<RichiestaGetGenerica, RispostaGetGenerica> {
    ...
    public RispostaGetGenerica handleRequest(RichiestaGetGenerica request, Context context) {
    		...
    		String codiceOrdine = request.getCodiceOrdine();
    		
    		if(codiceOrdine == null || codiceOrdine.equals("")) {
    			esito.setCodice(EsitoHelper.ESITO_KO_CODICE_ERRORE_GET);
    			esito.setMessage(className + " - " + EsitoHelper.ESITO_KO_MESSAGGIO_ERRORE_GET + " getOrdine - codice ordine non specificato ");
    		...
    		if(client != null) {
    			...
    			Ordine ordine;
    			try {
    				ordine = mapper.load(Ordine.class, codiceOrdine);
    			} catch (Exception e) {
    				esito.setCodice(EsitoHelper.ESITO_KO_CODICE_ERRORE_GET);
    				esito.setMessage(className + " - " + EsitoHelper.ESITO_KO_MESSAGGIO_ERRORE_GET + " getAziende ");
