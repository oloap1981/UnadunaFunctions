...
public class UnaDunaGetModelli implements RequestHandler<RichiestaGetGenerica, RispostaGetGenerica> {
    ...
    public RispostaGetGenerica handleRequest(RichiestaGetGenerica request, Context context) {
    		...
    		if(client != null) {
    			...
    			try {
    				modelli = mapper.scan(Modello.class, expr);
    			} catch (Exception e) {
    				esito.setCodice(EsitoHelper.ESITO_KO_CODICE_ERRORE_GET);
    				esito.setMessage(className + " - " + EsitoHelper.ESITO_KO_MESSAGGIO_ERRORE_GET);
